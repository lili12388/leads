...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  let sheetsApiKey = ''; // user-provided; no key is shipped by default
  ...
  async function loadSheetsConfig() {
    const result = await chrome.storage.local.get(['sheetsConfig', 'sheetsSyncEnabled', 'sheetsSyncNoWebsiteOnly', 'sheetsDemoMode', 'sheetsApiKey']);
    ...
    const apiKeyEl = document.getElementById('sheets-api-key');
  ...
  async function toggleSheetsSync() {
    ...
    if (sheetsSyncEnabled) {
      if (!sheetsApiKey) {
        alert('Please enter your Sheets API key before enabling sync.');
  ...
  function initSheetsUI() {
    ...
    if (testBtn) {
      testBtn.addEventListener('click', async () => {
        ...
        if (!sheetsApiKey) {
          alert('Please enter your Sheets API key');
          return;
        }
    ...
    if (apiKeyEl) {
      apiKeyEl.addEventListener('input', async () => {
        sheetsApiKey = apiKeyEl.value.trim();
        await saveSheetsConfig();
      });
    }
